 ...
 * @property {boolean} [needsExclusiveRun] - Set true when the test cannot tolerate a sibling lane in flight — for an integration test, typically because it writes a fixed-name file to the shared fixture root that production auto-detection scans (e.g. CLAUDE.md, which addAIAssistantFiles picks up on every createConversation). Such a file cannot be namespaced behind a per-test prefix, so while it exists any sibling lane's createConversation injects a phantom context item. The Go runner schedules these tests sequentially after the parallel phase, with a fixture reset around each, so no sibling test is ever in flight. The same flag on a unit or extension suite means the same thing; listExclusiveTests() in integration-test-executor.js lists every reason that qualifies.
